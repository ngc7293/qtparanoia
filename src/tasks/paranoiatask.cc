...
void ParanoiaTask::run()
{
    ...
    for (Track* track : disk_->tracks()) {
        ...
        std::stringstream ss;
        ss << dir.absolutePath().toStdString() << "/" << std::setw(2) << std::setfill('0') << track->num() << " " << track->title().toStdString() << ".wav";

        std::ofstream file(ss.str(), std::ofstream::binary | std::ofstream::app | std::ofstream::out);
